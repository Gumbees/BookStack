...
pub async fn list(db: &PgPool, page_id: Option<i64>) -> Result<Vec<Comment>> {
    let comments = match page_id {
        Some(pid) => {
            sqlx::query_as::<_, Comment>("SELECT * FROM comments WHERE page_id = $1 ORDER BY id")
                .bind(pid)
                .fetch_all(db)
                .await?
        }
        None => {
            sqlx::query_as::<_, Comment>("SELECT * FROM comments ORDER BY id DESC LIMIT 500")
                .fetch_all(db)
                .await?
        }
    };
...
pub async fn get(db: &PgPool, id: i64) -> Result<Comment> {
...
pub async fn create(
    ...
    pages::fetch(db, page_id).await?;
    if let Some(parent) = parent_id {
        let parent_comment = get(db, parent).await?;
...
pub async fn update(db: &PgPool, user_id: i64, id: i64, markdown_body: &str) -> Result<Comment> {
    get(db, id).await?;
    let html = markdown::render(markdown_body);
    Ok(sqlx::query_as::<_, Comment>(
